...
- REQ-062: The public repository provides a Russian-first README and responsive text-led GitHub Pages site that positions self-hosting and owner-controlled provider access first, covers personal, family and small-company use, documents the complete implemented feature set and installation, provides private vulnerability reporting guidance and full-history secret scanning in CI, and includes a validated agent skill for secure deployment and provider onboarding.
- REQ-063: Public documentation contains no personal mailbox/chat data, explains the full product and safety boundaries through concrete text scenarios without decorative product mockups, uses accessible high-contrast light, blue and dark sections, and is distributed under the MIT license. Successful interactive Web actions expose a localized, accessible, transient outcome notification.
...
- REQ-066: An open local task with a future due time owns exactly one linked Web Push delivery. Due-time changes move it, title changes update its text without repeating a delivered alert, completion/deletion removes it, reopening restores it, and existing future tasks are backfilled. Every authenticated Web view visibly reports this browser's notification state and offers a user-initiated enable/recovery action without exposing subscription secrets or prompting permission on page load.
...
- REQ-075: Every persisted chat message displays its localized send time. Timer creation repeats the timer name, duration and local end time. Successful named-timer delivery marks that timer finished, appends a timestamped completion message to chat exactly once, and becomes visible through bounded background chat synchronization so concurrent timers remain distinguishable.
...
### REQ-077 — Viewport-чат и ограниченная история

- На desktop и mobile основной экран чата занимает доступную высоту viewport без общей прокрутки; прокручивается только история сообщений.
- Desktop-навигация сворачивается в панель иконок, а примеры команд скрываются при недостаточной ширине.
- Заголовок чата отсутствует; на широком экране показывается не более четырёх примеров команд.
...
- На экранах шириной от 320 px все пользовательские разделы помещаются в viewport без горизонтальной прокрутки.
- Длинный текст переносится, формы и фильтры сжимаются или складываются в колонку, а действия остаются доступны.

### REQ-080 — Адресная доставка Web Push

- Тест уведомления отправляется только в текущую браузерную подписку и сообщает результат именно этого push endpoint.
- Worker хранит результат отдельно для каждого устройства, повторяет только неуспешные доставки и удаляет подписки, отклонённые push-сервисом с `404/410`.
- Успех Windows/Android не скрывает ошибку Apple; принятие push-сервисом не трактуется как подтверждение показа операционной системой.
- Payload поддерживает Declarative Web Push для современных iOS и сохраняет совместимость с service worker.

### REQ-081 — Контур обязательств

- По явному запросу пользователя AI анализирует не более 15 входящих и 15 отправленных писем за 30 дней и выделяет только явные категории «Я обещал», «Ждут от меня» и «Жду я».
- Анализ сверяет договорённости с открытыми задачами и ближайшим календарём, помечая явно не покрытые планом обязательства.
- Метаданные ограничены по размеру, считаются недоверенными, обрабатываются с `store=false` и не сохраняются в отчёте или журнале.
- Отчёт не выполняет действий: переход «Разобрать» создаёт только черновик команды в чате, а любые внешние изменения проходят существующее подтверждение.

### REQ-082 — Server-to-server API записи на встречи

- Владелец включает запись и создаёт отзываемый API-ключ в настройках; открытое значение показывается один раз, в базе хранится только SHA-256 хеш.
- Сайт после собственной проверки лида запрашивает слоты и создаёт встречу по Bearer API без публичной формы AI Planner, CAPTCHA и подтверждения email.
- Длительность, рабочие часы, горизонт, минимальное предупреждение, буферы и дневной лимит определяются только настройками владельца. Booking использует только подключённый Google Календарь; отдельный default ВКС для записи выбирается между Google Meet, Яндекс Телемостом, Zoom и отсутствием видеосвязи.
- Все времена API имеют UTC offset; доступность возвращается в запрошенной валидной IANA timezone, а создание повторно проверяет календарь непосредственно перед записью.
- Один `lead_id` может получить не более трёх успешно созданных встреч. Ошибки и занятые слоты попытку не расходуют; повтор с тем же `Idempotency-Key` возвращает прежний результат.
- Успешная запись создаёт и читает обратно событие Google, добавляет email лида участником и отправляет стандартное календарное приглашение. Отмена и перенос через booking API не поддерживаются.

### REQ-083 — Совместные задачи внутри сервера

- Владелец открывает задачу существующим пользователям того же экземпляра по точному email; публичных ссылок и каталога пользователей нет.
- Владелец и участники равноправно редактируют задачу и общий чек-лист через Web и чат. Назначенного исполнителя, ролей и рабочего процесса Jira нет.
- Только владелец управляет участниками и удаляет задачу; участник может самостоятельно покинуть её.
- Задача отображается участникам в общем списке, Today/Week и фильтре совместных задач. Последние изменения сохраняют автора и время.

### REQ-084 — Гибкий поиск локальных сущностей
