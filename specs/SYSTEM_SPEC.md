...
- Длительность, рабочие часы, горизонт, минимальное предупреждение, буферы, дневной лимит, календарь и видеосервис определяются только настройками владельца.
...
- Успешная запись создаёт и читает обратно событие провайдера, добавляет email лида участником и отправляет стандартное календарное приглашение. Отмена и перенос через booking API не поддерживаются.
