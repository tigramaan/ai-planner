...
Tasks may be shared only with an existing user on the same server by exact email through `POST /api/v1/tasks/{id}/participants`. Owners and participants can edit task fields and checklist items; only the owner can add/remove other participants or delete the task, while a participant may remove their own membership. `GET /api/v1/tasks` and Today/Week include owned and shared tasks. Responses expose the owner, current participants, checklist and the latest bounded activity history. There is no public sharing link, assignee or role workflow.
